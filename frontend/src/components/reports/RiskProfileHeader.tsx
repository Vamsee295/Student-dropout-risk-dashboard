...
interface RiskProfileHeaderProps {
    studentName?: string;
    studentId?: string;
    riskScore?: number;
}

export function RiskProfileHeader({ studentName = "Student", studentId = "N/A", riskScore = 0 }: RiskProfileHeaderProps) {
    const initials = studentName.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2);
    const email = `${studentId}@university.edu`;
    const riskPct = Math.round(riskScore);
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-gray-500 mb-2">
                            ID: #{studentId} • Risk Score: {riskPct}%
                        </p>
                ...
                <div className="flex items-center gap-6 rounded-xl bg-red-50 p-4 min-w-[300px]">
                    <div className="relative h-20 w-20 flex items-center justify-center">
                        ...
                        <span className="absolute text-xl font-bold text-red-700">{riskPct}%</span>
                    </div>
