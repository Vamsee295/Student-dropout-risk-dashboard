...
def get_faculty_exams(
    ...
    for assess, course in rows:
        ...
        result.append({
            ...
            "totalMarks": int(round(assess.total_marks)) if assess.total_marks else 50,
            "avgMarks": int(round(avg_row)) if avg_row else 0,
            "highestMarks": int(round(max_row)) if max_row else 0,
            "lowestMarks": int(round(min_row)) if min_row else 0,
            "passRate": pass_rate,
            "status": status,
    ...
    return [
        {
            ...
            "obtained_marks": int(round(sa.obtained_marks)) if sa.obtained_marks is not None else None,
